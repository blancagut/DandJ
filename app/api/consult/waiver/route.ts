...
export async function POST(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error("Waiver screening insert error:", error)
      return NextResponse.json({ error: "Failed to save waiver screening" }, { status: 500 })
    }

    return NextResponse.json({ success: true })
  } catch (err) {
    console.error("Waiver screening API error:", err)
