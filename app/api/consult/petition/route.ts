...
export async function POST(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error("Petition screening insert error:", error)
      return NextResponse.json({ error: "Failed to save petition screening" }, { status: 500 })
    }

    return NextResponse.json({ success: true })
  } catch (err) {
    console.error("Petition screening API error:", err)
