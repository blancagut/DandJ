...
export async function POST(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error("Work screening insert error:", error)
      return NextResponse.json({ error: "Failed to save work screening" }, { status: 500 })
    }

    return NextResponse.json({ success: true })
  } catch (err) {
    console.error("Work screening API error:", err)
