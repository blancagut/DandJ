...
export function PetitionsTab() {
  ...
  const handleDelete = async (id: string) => {
    ...
    const { data: check } = await supabase.from("petition_screenings").select("id").eq("id", id).maybeSingle()
  ...
  const handleBulkDelete = async (ids: string[]) => {
    ...
    if (e) { alert("Error deleting: " + e.message); return }
    // Verify deletion actually happened (RLS may silently block)
    const { data: check } = await supabase.from("petition_screenings").select("id").in("id", ids)
